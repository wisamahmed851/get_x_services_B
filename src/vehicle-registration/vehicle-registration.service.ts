...
export class VehicleRegistrationService {
    ...
    async create(dto: CreateVehicleRegistrationDto): Promise<VehicleRegistration> {
    ...
    async remove(id: string): Promise<void> {
        ...
        if (result.affected === 0) throw new NotFoundException(`Vehicle with ID ${id} not found`);
    }
}
